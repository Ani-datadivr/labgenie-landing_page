...
> **Keystatic needs no env var.** Cloud editing turns on automatically in any
> production build and the project slug is hardcoded in `keystatic.config.tsx` (see
> §3). The optional override `NEXT_PUBLIC_KEYSTATIC_CLOUD_PROJECT` is only for
> renaming the Cloud project later — leave it unset.
...
vercel --prod                                     # deploy
```
Keystatic Cloud (§3) needs **no env vars at all** here — the project slug is
hardcoded in `keystatic.config.tsx` and Cloud turns on automatically in production.
...
- Cloud editing is **already wired in code**: `keystatic.config.tsx` switches to
  Keystatic Cloud automatically in any production build (slug
  `website-designers/labgenie-landing` hardcoded). **No Vercel env var needed.**
- **One-time Keystatic Cloud setup (dashboard only):**

  1. At https://keystatic.cloud, the team + project
     **`website-designers/labgenie-landing`** exist, and the project is **connected to
     ...
     GitHub App on the repo for you — you don't manage any OAuth keys).
  2. In the project's settings, **add your Vercel domain(s)** to the allowed list
     (the `*.vercel.app` production URL, plus any custom domain). `localhost:3000`
     is allowed for local testing.
  3. **Invite your 1–2 editors by email** from the Keystatic Cloud dashboard.

- Push to `main` (or any deploy). Then `/keystatic` on the live site asks editors to
  sign in to Keystatic Cloud (email), and **Save commits straight to `main`** → Vercel
  rebuilds in ~1 min → the change shows on the Vercel URL for you to review. No
  branches, no PRs.
- Editing locally still works with **zero setup** at `localhost:3000/keystatic` under
  `npm run dev` (writes the JSON files directly; the running page is the live preview).
  See `KEYSTATIC.md`.
